...
void HondazE(){
	...
	for(nf=0; nf<chf; nf++){
		for(nt=0; nt<cht; nt++){
			sprintf(cn,"%s%ssp%c%c13.dat",nex,nsp,f[nf],t[nt]);
			f140=fopen(cn,"r");
			for(e=0; e<che; e++){
				for(a=0; a<cha; a++){
	...
	for(ndr=0; ndr<2; ndr++){
		for(nf=0; nf<chf; nf++){
			for(nt=0; nt<cht; nt++){
				...
				if(!nf){
					grsp[nf][nt][0].GetYaxis()->SetRangeUser(0.5,12.25);
					}
					else{
						grsp[nf][nt][0].GetYaxis()->SetRangeUser(0.5,7.2);
					}
				...
				TLegend* leg1=new TLegend(0.18,0.33,0.28,0.73);
				TLegend* leg2=new TLegend(0.28,0.33,0.38,0.73);
				...
				legname->Draw();
				c.SetLogx();
				sprintf(cn,"../pictures/spectra/H11x%c%crtzE-1+4.eps",f[nf],t[nt]);
