...
void ISU13pE(){
	...
	const char nsp[5]="H11x",*nex="../../fortran/Output/specexp/";
	...
	for(ndr=0; ndr<2; ndr++){
		for(nf=0; nf<chf; nf++){
			for(nt=0; nt<cht; nt++){
				...
				grsp[nf][nt][0].GetXaxis()->SetRangeUser(1e-1,1.1e11);
				...
				gStyle->SetPadLeftMargin(0.075);
				if(!nf){
					grsp[nf][nt][0].GetYaxis()->SetRangeUser(1e-8,3e-2);
					}
					else{
						grsp[nf][nt][0].GetYaxis()->SetRangeUser(1e-7,2e-1);
					}
				grsp[nf][nt][0].GetYaxis()->SetRangeUser(1e-12,2e-1);
				...
				c.SetLogx();
				c.SetLogy();
				sprintf(cn,"../pictures/spectra/ISUx%c%cE-1+11.eps",f[nf],t[nt]);
