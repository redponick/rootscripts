...
void testnsl(){
	...
	for(nh=0; nh<chh; nh++){
		for(nf=0; nf<chf; nf++){
			for(nt=0; nt<cht; nt++){
				sprintf(cn,"%s%s%c%s%c%c.dat",nex,"512",h[nh],nsp,f[nf],t[nt]);
				fold=fopen(cn,"r");
				sprintf(cn,"%s%s%c%s%c%c.dat",nex,"001",h[nh],nsp,f[nf],t[nt]);
				fnew=fopen(cn,"r");
				sprintf(cn,"%s%s%c%s%c%c.dat",nex,"512_001",h[nh],nsp,f[nf],t[nt]);
				ferr=fopen(cn,"w");
				for(e=0; e<che; e++){
					...
					err[nh][nf][nt][e]=fabs(spnew[nh][nf][nt][e]-spold[nh][nf][nt][e])/spold[nh][nf][nt][e];
	...
	gr[1][0][1].GetXaxis()->SetTitle("#font[132]{E_{#nu} (GeV)}");
	gr[1][0][1].GetXaxis()->SetRangeUser(1e-1,1.1e2);
	gr[1][0][1].GetYaxis()->SetTitle("#font[132]{relative error}");
	gr[1][0][1].GetYaxis()->SetRangeUser(0,1.5e-3);
	gr[1][0][1].Draw("AC");
	for(nh=0; nh<chh; nh++){
		for(nf=0; nf<chf; nf++){
			for(nt=0; nt<cht; nt++){
	...
	leg->Draw("SAME");
	c.SetLogx();
	sprintf(cn,"../pictures/slayers/512vs001_%s.eps",nsp);
