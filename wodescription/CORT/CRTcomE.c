...
void CRTcomE(){
	...
	const int chd=2,chh=2,chf=2,cht=2,che=100,cha=2;
	const char nsp[5]="CRTn",*nex="../../fortran/Output/";
	const char den[chd][4]={"vac","2lm"},h[chh]={'n','i'},f[chf]={'e','m'},t[cht]={'n','a'};
	char *name,cn[256];
	FILE *f140=0;
...
//	const int col[cht]={4,1},sh[cht]={7,1},w[cht]={2,1};
	...
	for(nd=0; nd<chd; nd++){
		for(nh=0; nh<chh; nh++){
			for(nf=0; nf<chf; nf++){
				for(nt=0; nt<cht; nt++){
					sprintf(cn,"%s%s%c%s%c%c.dat",nex,den[nd],h[nh],nsp,f[nf],t[nt]);
					f140=fopen(cn,"r");
					for(e=0; e<che; e++){
						for(a=0; a<cha; a++){
	...
	for(ndr=0; ndr<2; ndr++){
		...
		for(nh=0; nh<chh; nh++){
			for(nf=0; nf<chf; nf++){
				for(nt=0; nt<cht; nt++){
					for(e=0; e<che; e++){
						grsp[nf][nt].SetPoint(e,sp[0][nh][nf][nt][e][0],sp[1][nh][nf][nt][e][1]/sp[0][nh][nf][nt][e][1]);
					}
				}
			...
			c.SetLogx();
	//		c.SetLogy();
			sprintf(cn,"../pictures/spectra/%srat%c.eps",nsp,h[nh]);
