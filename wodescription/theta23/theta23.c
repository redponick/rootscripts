...
//	for(nt=0; nt<cht; nt++){
		for(ndr=0; ndr<2; ndr++){
			for(nf=0; nf<chf-1; nf++){
				...
				leglep->AddEntry((TObject*)0,"#font[132]{IH}","");
				...
				legv->AddEntry(&grve[nf][1],"#font[132]{M_{A}=0.964#pm0.044 GeV (world averaged value)}","FL");
				legv->AddEntry(&grse[2][nf],"#font[132]{M_{A}^{eff}}","FL");
				legv->SetBorderSize(0);
				legv->Draw();
				c.SetLogx();
				sprintf(cn,"%st23%c%s%c%c_6.eps",nod,net[nn],den[nd],h[nh],f[nf]);//,t[nt]);
