...
void theta2311(){
	...
	const char *nip="../../fortran/Output/QESintP/",*nod="../pictures/qesintp/";
...
//	for(nt=0; nt<cht; nt++){
		for(ndr=0; ndr<2; ndr++){
			for(nf=0; nf<chf-1; nf++){
				...
				grs[1][nf].GetXaxis()->SetTitle("#font[132]{P_{lep} (GeV)}");
				grs[1][nf].GetYaxis()->SetTitle("#font[132]{R}");
				grs[1][nf].GetXaxis()->SetRangeUser(9e-2,5e1);
				if(nf==2){
					grs[1][nf].GetYaxis()->SetRangeUser(-50,3000);
					}else{
						grs[1][nf].GetYaxis()->SetRangeUser(0.76,1.28);
					}
				grs[1][nf].SetLineColor(0);
				grs[1][nf].Draw("AL");
				for(ns=1; ns<chs-1; ns++){
					grse[ns][nf].SetFillColor(5);
					grse[ns][nf].Draw("3SAME");
				}
				for(ns=1; ns<chs-1; ns++){
					grs[ns][nf].SetLineColor(1);
					grs[ns][nf].SetLineWidth(w[ns]);
					grs[ns][nf].SetLineStyle(sh[ns]);
					grs[ns][nf].Draw("CSAME");
				}
				for(nv=0; nv<chv; nv++){
					if(nv==3){
				...
				leglep->Draw();
				TLegend* legv=new TLegend(0.5,0.75,0.98,0.95);
				legv->AddEntry(&grv[nf][3],"#font[132]{M_{A}=1.2 GeV (K2K best fit, SK default)}","L");
				legv->AddEntry((TObject*)0,"#font[132]{M_{A}^{eff} with:}","");
				legv->AddEntry(&grs[1][nf],"#font[132]{sin^{2}#theta_{23}=0.331}","L");
				legv->AddEntry(&grs[2][nf],"#font[132]{sin^{2}#theta_{23}=0.386}","L");
				legv->AddEntry(&grs[3][nf],"#font[132]{sin^{2}#theta_{23}=0.637}","L");
				legv->SetBorderSize(0);
				legv->Draw();
				c.SetLogx();
				sprintf(cn,"%st23%c%s%c%c_11.eps",nod,net[nn],den[nd],h[nh],f[nf]);//,t[nt]);
